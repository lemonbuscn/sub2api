...
func (s *FrontendServer) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if cleanPath == "index.html" || !s.fileExists(cleanPath) {
			// Try local override first (e.g., data/public/lemonfly-home.css)
			if s.tryServeOverride(c, cleanPath) {
				return
			}
			// Root path or /home: serve custom landing page if exists
			if cleanPath == "index.html" || cleanPath == "home" {
				s.serveIndexHTML(c)
				return
			}
			// Other SPA routes (e.g., /login, /dashboard): serve embedded index.html
			s.serveEmbeddedIndexHTML(c)
			return
		}
...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	// Check for custom landing page override in data/public/
	if s.overrideDir != "" {
		overrideIndex := filepath.Join(s.overrideDir, "index.html")
		if _, err := os.Stat(overrideIndex); err == nil {
			c.File(overrideIndex)
			c.Abort()
			return
		}
	}
...
// serveEmbeddedIndexHTML serves the embedded SPA index.html without custom landing page override.
// Used for SPA routes like /login, /dashboard, etc.
func (s *FrontendServer) serveEmbeddedIndexHTML(c *gin.Context) {
	nonce := middleware.GetNonceFromContext(c)
